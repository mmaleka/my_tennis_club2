    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.FILES.get("media", None) is not None:
            ...
            if len(bounding_boxes_list) >= 7:
                (bb_digits, bb_alphabets) = sort.sortBB(
                    image, bounding_boxes_list)
                prediction_label_digits = serial.detect_digits(
                    bb_digits, image, ori_image)
                prediction_label_alphabets = "------"
                start_time5 = time.time()
                if len(bb_alphabets) == 6:
                    prediction_label_alphabets = serial.detect_alphabets(
                        bb_alphabets, image, ori_image)
                    print("alphabets process complete in: ",
                          time.time()-start_time5)
                    # counter +=1
                ...
                print("shellNo: ", prediction_label_digits,
                      " - Batch: ", prediction_label_alphabets)
                serial_number = str(prediction_label_digits) + \
                    '-'+prediction_label_alphabets
